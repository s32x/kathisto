...
// noRenderKey is used in the queryparam of an forced un-rendered request to
// avoid recursive rendering
const noRenderKey = "norender"

// Chrome is a struct containing all dependencies needed to perform HTML
// rendering and response caching using Headless Chrome. It satisfies the
// Renderer interface
type Chrome struct {
	userAgent string
...
// NewChromeRenderer generates a new Chrome Renderer that renders and stores
// rendered pages in a cache
func NewChromeRenderer(userAgent string) Renderer {
	return &Chrome{userAgent, make(map[string][]byte)}
}

// Render renders a page using Headless Chrome and returns the bytes
func (c *Chrome) Render(url string) ([]byte, error) {
	...
	// Construct a norender url to avoid recursive rendering
...
// stripErrors strips all prefixing errors from the rendered webpage
